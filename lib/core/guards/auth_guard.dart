...
class AuthGuard extends ChangeNotifier {
  ...
  String? redirect(BuildContext context, GoRouterState state) {
    ...
    final isOnAvaliacaoPage =
        state.uri.toString().startsWith(AppRoutes.avaliacao);

    // Rotas que não devem ser redirecionadas (ex: deep links de pagamento)
    final isOnProtectedPages =
        state.uri.toString().startsWith(AppRoutes.statusAluguel);
    ...
    if (isLoggedIn && usuario != null) {
      ...
      if (isOnAuthPages) {
        ...
        if (statusEndereco == null ||
            statusEndereco == StatusEndereco.rejeitado) {
      ...
      if (state.uri.toString() == AppRoutes.verificacaoTelefone &&
      ...
      if (state.uri.toString() == AppRoutes.verificacaoResidencia &&
          statusEndereco != null &&
          (statusEndereco == StatusEndereco.aprovado ||
              statusEndereco == StatusEndereco.emAnalise)) {
        return AppRoutes.home;
      }

      // Se está logado e verificado (aprovado), verificar se há avaliações pendentes
      if (!isOnProtectedPages &&
          !isOnAvaliacaoPage &&
          usuario.telefone != null &&
          statusEndereco == StatusEndereco.aprovado) {
        final avaliacoesPendentes = _ref.read(avaliacoesPendentesProvider);
        if (avaliacoesPendentes.hasValue &&
            avaliacoesPendentes.value!.isNotEmpty) {
          ...
          final avaliadoNome =
              primeiraAvaliacao['avaliadoNome'] as String? ?? 'Usuário';
