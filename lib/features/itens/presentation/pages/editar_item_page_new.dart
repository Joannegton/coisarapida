...
class _EditarItemPageState extends ConsumerState<EditarItemPage> {
  ...
  void _preencherCampos(Item item) {
    ...
    _fotosOriginais = List.from(item.fotos);
    _tipoItem = item.tipo;
  ...
  Future<void> _salvarAlteracoes() async {
    ...
    try {
      ...
      for (final fotoOriginal in _fotosOriginais) {
        if (!_fotosUrls.contains(fotoOriginal)) {
      ...
      if (novasFotos.isNotEmpty) {
        final urlsNovasFotos = await repository.uploadFotos(novasFotos, widget.itemId);
        fotosFinais.addAll(urlsNovasFotos);
      }

      // Deletar fotos removidas
      for (final fotoUrl in fotosParaDeletar) {
        await repository.deletarFoto(fotoUrl);
      }

      // Criar item atualizado
      final itemAtualizado = ItemModel(
        id: widget.itemId,
        proprietarioId: _itemOriginal!.proprietarioId,
        proprietarioNome: _itemOriginal!.proprietarioNome,
        proprietarioReputacao: _itemOriginal!.proprietarioReputacao,
        ...
        precoVenda: _tipoItem == TipoItem.venda || _tipoItem == TipoItem.ambos
            ...
            : null,
        valorCaucao: double.tryParse(_caucaoController.text.replaceAll(',', '.')),
        regrasUso: _regrasController.text.trim().isNotEmpty ? _regrasController.text.trim() : null,
        tipo: _tipoItem,
        estado: _estadoItem,
        disponivel: _itemOriginal!.disponivel,
        aprovacaoAutomatica: _aprovacaoAutomatica,
        criadoEm: _itemOriginal!.criadoEm,
        atualizadoEm: DateTime.now(),
        localizacao: _itemOriginal!.localizacao,
        avaliacao: _itemOriginal!.avaliacao,
        totalAlugueis: _itemOriginal!.totalAlugueis,
        visualizacoes: _itemOriginal!.visualizacoes,
      );

      // Atualizar no Firebase
      await repository.atualizarItem(itemAtualizado);

      // Invalidar o cache do provider para recarregar os dados
